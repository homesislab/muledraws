...
jQuery(document).ready(function() {
    const columns = [{
        ...
        template: function(row) {
            return `
                ${actionRow(row.id, row.status)}
                <a href="${path}upload/${row.id}" class="btn btn-sm btn-outline-primary btn-icon btn-icon-md" title="Upload Artwork"><i class="la la-upload"></i></a>
            `;
        },
    }];
